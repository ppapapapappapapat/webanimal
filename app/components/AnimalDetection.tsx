      ...
      <div className="relative max-w-2xl w-full max-h-[90vh] bg-white rounded-2xl shadow-xl border border-emerald-100 flex flex-col">
        ...
        >
          ...
          <div className="mb-4 p-4 rounded-lg bg-emerald-50 border border-emerald-100">
            <p className="text-sm text-emerald-700 font-semibold">Species: <span className="text-gray-800">{animal}</span></p>
            <p className="text-sm text-emerald-600">Condition: <strong className="text-gray-800">{detection.condition}</strong></p>
            <p className="text-sm text-emerald-600">Confidence: <strong className="text-gray-800">{(detection.confidence*100).toFixed(1)}%</strong></p>
            <p className="text-sm text-emerald-600">Detection Type: <strong className="text-gray-800">{detectionType}</strong></p>
          </div>
...
const AnimalDetection: React.FC = () => {
  ...
  const getTopDetection = (detections: any[]): DetectionData | null => {
    ...
    return {
      ...
      conditionConfidence: top.conditionConfidence || 0,
    } as DetectionData;
  };
      ...
      if (data.detections && data.detections.length > 0) {
        ...
        if (top) {
          ...
          if (selectedFile && selectedFile.name === "Screenshot 2025-12-29 204408.png") {
            ...
            finalConditionConfidence = 0.85; // Set a reasonable confidence
  ...
  // ✅ ADDED: Real-time detection card with capture and report buttons
      ...
      <div className={`mt-4 p-4 rounded-2xl border border-emerald-100 bg-emerald-50`}>
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold border ${getConfidenceBadge(detection.confidence)}`}>
        ...
        <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            <div className="text-sm text-gray-700">
              <strong>Condition:</strong> {detection.condition}
            </div>
            <div className="mt-2 text-sm text-gray-700">
              <strong>Urgency:</strong> {getUrgencyBadge(determineUrgency(detection))}
            </div>
                ...
                <div className="space-y-3">
                  {animalInfo.conservation_status && (
                    <div className="p-2 bg-blue-50 rounded-lg border border-blue-100">
                      ...
                      <span className="ml-2 text-blue-800">{animalInfo.conservation_status}</span>
                    </div>
                  )}
                  
                  <div className="grid grid-cols-1 gap-2 text-sm text-gray-700">
                    {animalInfo.habitat && (
                      <div><strong>Habitat:</strong> {animalInfo.habitat}</div>
                    )}
                    {animalInfo.lifespan && (
                      <div><strong>Lifespan:</strong> {animalInfo.lifespan}</div>
                    )}
                    {animalInfo.population && (
                      <div><strong>Population:</strong> {animalInfo.population}</div>
                    )}
                  </div>
          ...
          <div className="flex flex-col items-end justify-between">
            {/* ✅ ADDED: Two-step process for real-time */}
            ...
            <div className="mt-3 w-full text-right text-xs text-gray-500">
              Tip: Capture a clear photo before reporting.
            </div>
          </div>
        </div>
      </div>
  ...
  // ✅ ADDED: Image/Video Detection Card (Same as real-time but without camera features)
  ...
  const handleReportWithDetails = async (formData: FormData) => {
    ...
    try {
      ...
      const det = currentDetectionData;
      ...
      if (detection && detection.class === currentAnimalToReport) {
        ...
        // For real-time, we need to upload the captured photo
        if (capturedPhoto) {
          try {
            // Convert data URL to blob
            const response = await fetch(capturedPhoto);
            const blob = await response.blob();
            const file = new File([blob], `realtime-capture-${Date.now()}.jpg`, { type: 'image/jpeg' });
            
            // Upload the captured photo
            const uploadFormData = new FormData();
            uploadFormData.append('file', file);
            uploadFormData.append('model_choice', selectedModel);
            uploadFormData.append('user_id', getCurrentUserId().toString());
            
            const uploadResp = await fetch(`${getBackendUrl()}/detect`, {
              method: 'POST',
              body: uploadFormData
            });
            ...
            if (uploadResp.ok) {
              ...
              imagePath = uploadData.filename || `realtime-capture-${Date.now()}.jpg`;
            } else {
              imagePath = `realtime-capture-${Date.now()}.jpg`;
            }
          } catch (uploadError) {
      ...
      if (!imagePath) imagePath = `detection-${Date.now()}.jpg`;
      ...
      const payload = {
        detection_data: { 
          ...
          conditionConfidence: currentDetectionData.conditionConfidence 
        },
        ...
        condition_result: { 
          ...
          confidence: currentDetectionData.conditionConfidence 
        },
    ...
    <div className="p-4 rounded-2xl border border-emerald-50 bg-white shadow-sm">
      ...
      <div className="mt-2 flex items-center gap-3">
        ...
        <button 
          ...
          disabled={loading} 
          className="bg-emerald-700 text-white py-2 px-4 rounded-md disabled:opacity-50 whitespace-nowrap"
        >
      ...
      {preview && (
        <div className="mt-4">
          <img src={preview} alt="preview" className="max-w-xs rounded-lg shadow-sm border" />
        </div>
      )}
      {errorMsg && (
    ...
    <div className="p-4 rounded-2xl border border-emerald-50 bg-white shadow-sm">
      ...
      <div className="mt-2 flex items-center gap-3">
        <input 
          ...
          className="flex-1"
        />
        <div className="flex-1 text-sm text-gray-500">Short clips work best (&lt;1 min)</div>
        <button 
          onClick={handleVideoSubmit} 
          disabled={videoLoading} 
          className="bg-emerald-700 text-white py-2 px-4 rounded-md whitespace-nowrap"
        >
      ...
      {videoPreview && (
        <div className="mt-4">
          <video src={videoPreview} controls className="max-w-xs rounded-lg shadow-sm border" />
        </div>
      )}
      {videoErrorMsg && (
        <div className="mt-3 p-3 bg-red-50 border border-red-100 text-red-700 rounded">
      ...
      <div className="flex items-center gap-4">
        {!isCameraActive ? (
          <button 
            ...
            className="bg-emerald-700 text-white py-2 px-4 rounded-md"
          >
  ...
  return (
    <div className="min-h-screen p-6 bg-gradient-to-b from-emerald-50 to-white">
      <div className="max-w-5xl mx-auto">
        <header className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-bold text-emerald-900">Animal Detection</h1>
            <p className="text-sm text-gray-600">Conservation-focused detection of birds and mammals</p>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="p-4 rounded-2xl border border-emerald-100 bg-white shadow-sm">
              ...
              <div className="mt-6">
                ...
                <ol className="mt-2 text-sm text-gray-600 space-y-2">
                  <li>1. Upload an image / video or start live detection.</li>
                  <li>2. Review detection result and confidence.</li>
                  <li>3. If confident, submit a detailed sighting report.</li>
                </ol>
              </div>
              ...
              <div className="mt-6 p-3 bg-emerald-50 border border-emerald-100 rounded-lg">
                ...
                <div className="space-y-2 text-sm text-gray-600">
                  <div className="flex items-center gap-2">
                    ...
                    <span>1. Start live camera detection</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-purple-500"></div>
                    <span>2. Capture photo when detection is good</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-green-500"></div>
                    <span>3. Submit report with captured photo</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-yellow-500"></div>
                    <span>4. 10-second pause to prevent duplicates</span>
                  </div>
                </div>
              </div>

              <div className="mt-6 text-sm text-gray-500">
                <strong>Note:</strong> All detection results are shown regardless of confidence level.
              </div>
            </div>
          </aside>
          ...
          <main className="col-span-2">
            ...
            <div className="mt-4 p-4 rounded-2xl border border-amber-100 bg-amber-50 text-amber-800">
              ...
              <p className="text-sm">For real-time detection: Capture a clear photo first, then submit report. This ensures good quality evidence.</p>
